...
- [make_quantile_map_array](https://github.com/iiasa/rime/blob/rimeX/rimeX/preproc/quantilemaps.py#L30-L224) : create the quantile map as indicated in the picture.
- [make_quantilemap_prediction](https://github.com/iiasa/rime/blob/rimeX/rimeX/preproc/quantilemaps.py#264-L333) : use the output of `make_quantile_map_array` to make predictions.

The corresponding command line `rime-pre-quantilemap` handles the creation of the quantile map (`make_quantile_map_array`), not the predictions themselves, which
msut be done calling `make_quantilemap_prediction` directly in python.
